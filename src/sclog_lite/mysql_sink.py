"""MySQL Sink Module for sclog_lite.

This module provides a custom sink for loguru that handles writing log records
to a MySQL database asynchronously, in batches, using connection pooling
...
class MySQLSink:
    """A custom loguru sink for writing logs to MySQL in asynchronous batches.
    ...
    records to a MySQL database. It uses DBUtils PooledDB for connection pooling
    and pymysql as the underlying driver.

    Attributes:
        _host (str): MySQL host.
        _port (int): MySQL port.
        _user (str): MySQL user.
        _password (str): MySQL password.
        _database (str): MySQL database name.
    ...
    def _handle_queue_full(self, record: Dict[str, Any]) -> None:
        ...
        """
        err_msg = "[sclog_lite] Queue full! MySQL Sink dropped log record.\n"
        try:
    ...
    def _handle_internal_error(self, exception: Exception, context: str = "") -> None:
        ...
        """
        try:
            err_msg = f"[sclog_lite] MySQL Sink Error during {context}: {exception}\n"
            sys.stderr.write(err_msg)
            sys.stderr.flush()
        except Exception:
            pass
    ...
    def _write_to_fallback(self, batch: List[Dict[str, Any]], exception: Exception) -> None:
        ...
        Args:
            batch: A list of log records that failed to write to MySQL.
